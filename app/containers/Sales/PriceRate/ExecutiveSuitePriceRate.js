...
import { ExecutiveSuiteEditableCellDemo } from "./";
...
class Pricing extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      onClickButton: "",
      priceRateId: "",
      dynamicPremierPrice: "659.60",
      dynamicPremierLeisurePrice: "698.40",
    };
  }
  ...
  async componentDidMount(){
    try {
      setInterval(async () => {
        const dynamicPriceApi = await fetch(SERVER_PREFIX + "/pricerates/dynamicroomrate/Executive Suite");
  ...
  render() {
    const title = brand.name + " - Executive Suite Pricing";
    ...
    return (
      <div>
        ...
        >
          <div>
            <ExecutiveSuiteEditableCellDemo handleClickOpenSlide={this.handleClickOpenSlide} onClickButton={this.state.onClickButton} priceRateId={this.state.priceRateId} />
          </div>
        </PapperBlock>
        <Dialog
          open={openSlide}
