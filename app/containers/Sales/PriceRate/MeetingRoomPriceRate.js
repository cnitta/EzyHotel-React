...
import { MeetingRoomEditableCellDemo } from "./";
...
class Pricing extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      onClickButton: "",
      priceRateId: "",
      standardPrice: "823.20",
      leisurePrice: "882.00",
      corporatePrice: "940.80"
    };
  }
  ...
  async componentDidMount(){
    try {
      setInterval(async () => {
        const facilityPriceApi = await fetch(SERVER_PREFIX + "/pricerates/facilityrate/Meeting Room");
  ...
  render() {
    const title = brand.name + " - Meeting Room Pricing";
    ...
    return (
      <div>
        ...
        >
          <div>
            <MeetingRoomEditableCellDemo handleClickOpenSlide={this.handleClickOpenSlide} onClickButton={this.state.onClickButton} priceRateId={this.state.priceRateId} />
          </div>
        </PapperBlock>
        <Dialog
          open={openSlide}
