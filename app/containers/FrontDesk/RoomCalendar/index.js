...
import events from "../../../api/FrontDesk/roomBookingData";
...
class Calendar extends React.Component {
  ...
  reloadData() {
    Api.viewCalendar()
      .done(result => {
        console.log("Before RoomCalendar data");
        console.log(this.state.bookings);
        this.setState({
          bookings: result
        });

        let testingBookingList = [];
        for (let n = 0; n < this.state.bookings.length; n++) {
          testingBookingList[n] = this.state.bookings[n];
          this.state.testBookings[n] = testingBookingList[n];
        }
        //console.log("Test duplicate booking: " + this.state.testBookings);

        console.log("After RoomCalendar data");
        console.log(this.state.bookings);
      })
      .fail(() => {
  ...
  render() {
    ...
    //event
    const { anchorEl, anchorPos, event, testBookings, bookings } = this.state;
    //console.log("Test booking at index page: " + this.state.testBookings);
    const {
      classes,
      eventData,
      openFrm,
