...
import MainTableForm from "../../Tables/tableParts/MainTableForm";
...
class CrudTableForm extends React.Component {
  componentDidMount() {
    ...
    fetch(SERVER_PREFIX + "/facilities/query?hotelId=2")
  ...
  sendValues = values => {
    ...
    if (values.get("hotelId")) {
      setTimeout(() => {
        const putRequest = new Request(
          SERVER_PREFIX + "/facilities/" + values.get("hotelId"),
          {
            method: "PUT",
      ...
      setTimeout(() => {
        const postRequest = new Request(SERVER_PREFIX + "/facilities/", {
          method: "POST",
          body: JSON.stringify(values),
          headers: { "Content-Type": "application/json" }
        });
