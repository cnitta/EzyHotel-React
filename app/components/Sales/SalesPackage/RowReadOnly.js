...
class RowReadOnly extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.onClickButton != this.props.onClickButton) {
      ...
      if (
        ...
        this.props.item.get("salesPackageId") == this.props.salesPackageId
      ) {
        //console.log(prevProps);
        //console.log(this.state);
        this.props.removeRow(this.props.item, this.props.branch);
        const deleteRequest = new Request(SERVER_PREFIX + "/salespackages/" + this.props.item.get("salesPackageId"), { method: 'DELETE'})
  ...
  render() {
    ...
    const eventDel = () => {
      ...
      this.props.handleClickOpenSlide(item.get("salesPackageId"), item, branch);
    };
    const eventEdit = () => {
      this.props.updateImagePreview(item.get("salesPackageId"));
      editRow(item, branch);
      //console.log(item.get("salesPackageId"))
    };
    const renderCell = dataArray => dataArray.map((itemCell, index) => {
      if (itemCell.name == "startDate"){
        return (
