...
import PresidentSuiteEditableCell from "./PresidentSuiteEditableCell";
...
class Row extends React.Component {
  constructor(props) {
    ...
    fetch(SERVER_PREFIX + "/pricerates/rackrate/President Suite")
      ...
      if (item.get("priceRateId")) {
        ...
        if (item.get("rateTitle") == '' || item.get("basePrice") < 88 || item.get("percentageMarkup") < 10 || item.get("percentageMarkup") > this.state.rackPercentageMarkup) {
          ...
          const postRequest = new Request(
            SERVER_PREFIX + "/pricerates/roomtype/President Suite",
            {
              method: "POST",
              body: JSON.stringify(item),
              headers: { "Content-Type": "application/json" }
    ...
    return (
      <tr className={item.get("edited") ? css.editing : ""}>
        ...
        <TableCell padding="none">
          ...
          {item.get("rateTitle") == "Premier" || item.get("rateTitle") == "Premier Leisure" || item.get("rateTitle") == "Rack" ? "" : (
            <IconButton
