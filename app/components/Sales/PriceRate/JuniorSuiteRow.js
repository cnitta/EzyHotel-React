...
import JuniorSuiteEditableCell from "./JuniorSuiteEditableCell";
...
class Row extends React.Component {
  constructor(props) {
    ...
    fetch(SERVER_PREFIX + "/pricerates/rackrate/Junior Suite")
      ...
      if (item.get("priceRateId")) {
        ...
        if (item.get("rateTitle") == '' || item.get("basePrice") < 88 || item.get("percentageMarkup") < 10 || item.get("percentageMarkup") > this.state.rackPercentageMarkup) {
          ...
          const postRequest = new Request(
            SERVER_PREFIX + "/pricerates/roomtype/Junior Suite",
            {
              method: "POST",
              body: JSON.stringify(item),
              headers: { "Content-Type": "application/json" }
