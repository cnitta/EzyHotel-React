  ...
  render() {
  	...
  	const renderCell = (dataArray, keyArray) => keyArray.map((itemCell, index) => {
  		if (itemCell.id === "market") {
  			return (
  				<TableCell padding="dense" align={itemCell.numeric ? "right" : "left"} key={index.toString()}>
  		...
  		if (itemCell.id === "stats") {
  			return (
  				<TableCell padding="dense" align={itemCell.numeric ? "right" : "left"} key={index.toString()} className={classes.chartTable}>
  		...
  		return (
  			<TableCell padding="dense" align={itemCell.numeric ? "right" : "left"} key={index.toString()}>{dataArray[itemCell.id]}</TableCell>
  		);
  	});
  	return (
  		<PapperBlock noMargin title="Marketplace" icon="ios-basket-outline" whiteBg desc="Explore more than 10 crytocurrency markets.">
  			<div className={classes.rootTable}>
  				...
  				<div className={classes.tableWrapper}>
  					<Table padding="dense" className={classNames(classes.table, classes.stripped, classes.hover)}>
  						<EnhancedTableHead
  							order={order}
  							orderBy={orderBy}
