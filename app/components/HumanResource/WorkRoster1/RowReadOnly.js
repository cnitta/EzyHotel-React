...
import AddIcon from "@material-ui/icons/People";
  ...
  render() {
    ...
    const eventDel = () => {
      ...
      const deleteRequest = new Request(
        SERVER_PREFIX + "/workrosters/" + item.get("workRosterId"),
        { method: "DELETE" }
      );
          ...
          return (
            <TableCell padding="none" key={index.toString()}>
              {item.get(itemCell.name) !== undefined
                ? itemCell.name == "startDateTime" ||
                  itemCell.name == "endDateTime" ||
                  itemCell.name == "createDateTime"
                  ? moment(item.get(itemCell.name).toString())
                    .format("DD-MM-YYYY")
