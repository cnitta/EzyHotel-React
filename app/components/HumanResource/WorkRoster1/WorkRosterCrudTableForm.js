...
import WorkRosterFloatingPanel from "./WorkRosterFloatingPanel";
...
class CrudTableForm extends React.Component {
  ...
  sendValues = values => {
    ...
    if (values.get("workRosterId")) {
      setTimeout(() => {
        console.log("put values" + values);
        const putRequest = new Request(
          SERVER_PREFIX + "/workrosters/" + values.get("workRosterId"),
          {
            method: "PUT",
      ...
      setTimeout(() => {
        const postRequest = new Request(SERVER_PREFIX + "/workrosters", {
          method: "POST",
          body: JSON.stringify(values),
          headers: { "Content-Type": "application/json" }
        });
