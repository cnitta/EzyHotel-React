...
import MainTableForm from "./EstCleaningTimeParts/MainTableForm";
...
class CrudTableForm extends React.Component {
  componentDidMount() {
    ...
    fetch(SERVER_PREFIX + "/housekeepingSOP")
      .then(res => res.json())
      .then(findresponse => {
  ...
  sendValues = values => {
    ...
    if (values.get("housekeepingSOPId")) {
      console.log("update values" + values);
      setTimeout(() => {
        const putRequest = new Request(
          SERVER_PREFIX + "/housekeepingSOP/" + values.get("housekeepingSOPId"),
          {
            method: "PUT",
      ...
      setTimeout(() => {
        const postRequest = new Request(SERVER_PREFIX + "/housekeepingSOP/", {
          method: "POST",
          body: JSON.stringify(values),
          headers: { "Content-Type": "application/json" }
        });
